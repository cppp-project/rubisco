...
from repoutils.config import COPY_BUFSIZE, DEFAULT_CHARSET
...
def extract_zip(
    ...
    with zipfile.ZipFile(file, "r") as fp:
        ...
        task_name = format_str(
            ...
            fmt={"file": str(file), "path": str(dest)},
        )
        call_ktrigger(
            IKernelTrigger.on_new_task,
            task_name=task_name,
...
def compress_tarball(  # pylint: disable=too-many-arguments
    ...
    task_name = format_str(
        _(
            "Compressing '[underline]${{file}}[/underline]' to "
            "'[underline]${{path}}[/underline]' as '${{type}}' ..."
        ),
        fmt={
            "file": str(src),
...
def compress_zip(  # pylint: disable=too-many-arguments
    ...
    task_name = format_str(
        _(
            "Compressing '[underline]${{file}}[/underline]' to "
            "'[underline]${{path}}[/underline]' as 'zip' ..."
        ),
        fmt={"file": str(src), "path": str(dest)},
    )
...
def compress_7z(  # pylint: disable=too-many-arguments
    ...
    task_name = format_str(
        _(
            "Compressing '[underline]${{file}}[/underline]' to "
            "'[underline]${{path}}[/underline]' as '7z' ..."
        ),
        fmt={"file": str(src), "path": str(dest)},
    )
    ...
    with fsrc:
        ...
        with open(dest, "wb") as fdst:
            if fsize > COPY_BUFSIZE * 50:
                task_name = format_str(
                    _(
                        "Compressing '[underline]${{file}}[/underline]'"
                        " to '[underline]${{path}}[/underline]'"
                        " as '${{type}}' ..."
                    ),
                    fmt={
